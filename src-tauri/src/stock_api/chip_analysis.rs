...
impl ChipState {
    ...
    fn update_fixed_decay(&mut self, data: &StockData, factor_precision: f64, decay_factor: f64) {
        ...
        let avg_price = (data.open + data.close) / 2.0;
    ...
    fn update_dynamic_decay(&mut self, data: &StockData, factor_precision: f64, decay_coefficient: f64, dist_type: &DistributionType) {
        ...
        let remain_factor = 1.0 - turnover_rate * decay_coefficient;
        let new_chip_factor = turnover_rate * decay_coefficient;
            ...
            *self.chips.entry(price_int).or_insert(0.0) += new_chip_factor;
            ...
            match dist_type {
                ...
                DistributionType::Triangular => {
                    // Triangular distribution: centered at average price
                    let avg_price = (data.open + data.high + data.low + data.close) / 4.0;
                    let avg_int = (avg_price * factor_precision).round() as i64;
                    
                    // Calculate triangular weights
                    let total_weight = (high_int - low_int + 1) as f64 / 2.0;
                    
                    for p in low_int..=high_int {
                        let weight = if p <= avg_int {
                            // Left side: weight increases from low to avg
                            if avg_int > low_int {
                            ...
                            // Right side: weight decreases from avg to high
                            if high_int > avg_int {
                        ...
                        let mass_per_unit = new_chip_factor * weight / total_weight;
                        *self.chips.entry(p).or_insert(0.0) += mass_per_unit;
                    }
                }
            }
    ...
    fn calculate_metrics(&self, current_price: f64, factor_precision: f64) -> (f64, f64, f64, f64, f64) {
        if self.chips.is_empty() {
        ...
        // 1. Peak Price (Mode)
        // Simple mode: bin with max mass.
        // Better: smooth or find dense area. Simple mode for now.
        ...
        let concentration_90 = if p_95 + p_5 > 0.0 { (p_95 - p_5) / (p_95 + p_5) } else { 0.0 };
        let concentration_70 = if p_85 + p_15 > 0.0 { (p_85 - p_15) / (p_85 + p_15) } else { 0.0 };
        
        (peak_price, profit_ratio, concentration_90, concentration_70, average_cost)
    }
}
...
pub fn calculate_chip_distribution(
    ...
    if let Some(first) = data.first() {
        match decay_method {
            DecayMethod::Fixed => {
                // Initialize with volume at first day's price range
                let low_int = (first.low * factor).round() as i64;
                let high_int = (first.high * factor).round() as i64;
                let avg_price = (first.open + first.close) / 2.0;
            ...
            DecayMethod::Dynamic => {
                // Initialize with 100% mass at first day's average price
                let avg_price = (first.open + first.high + first.low + first.close) / 4.0;
    ...
    for d in data {
        match decay_method {
            ...
            DecayMethod::Dynamic => {
                state.update_dynamic_decay(d, factor, decay_factor, &distribution_type);
            }
        }
        
        let (peak, profit, c90, c70, avg) = state.calculate_metrics(d.close, factor);
