      ...
      <div className="portfolio-stats">
        <div className="stat-item">
          ...
          <span className="stat-value">
            {t("common.currencySymbol")}{stats.initialCapital.toFixed(2)}
            <button
        ...
        <div className="stat-item">
          ...
          <span className="stat-value">{t("common.currencySymbol")}{stats.totalValue.toFixed(2)}</span>
        </div>
        <div className="stat-item">
          <span className="stat-label">{t("portfolio.totalCost")}:</span>
          <span className="stat-value">{t("common.currencySymbol")}{stats.totalCost.toFixed(2)}</span>
        </div>
        <div className="stat-item">
          <span className="stat-label">{t("portfolio.availableBalance")}:</span>
          <span className="stat-value">{t("common.currencySymbol")}{stats.availableBalance.toFixed(2)}</span>
        </div>
        <div className="stat-item">
          <span className="stat-label">{t("portfolio.positionValue") || "Position Value"}:</span>
          <span className="stat-value">{t("common.currencySymbol")}{stats.positionValue.toFixed(2)}</span>
        </div>
        <div className="stat-item">
          ...
          <span className={`stat-value ${stats.floatingProfit >= 0 ? "positive" : "negative"}`}>
            {stats.floatingProfit >= 0 ? "+" : ""}{t("common.currencySymbol")}{stats.floatingProfit.toFixed(2)} ({stats.floatingProfitPercent >= 0 ? "+" : ""}{stats.floatingProfitPercent.toFixed(2)}%)
          </span>
        </div>
        <div className="stat-item">
          <span className="stat-label">{t("portfolio.totalProfit")}:</span>
          <span className={`stat-value ${stats.totalProfit >= 0 ? "positive" : "negative"}`}>
            {stats.totalProfit >= 0 ? "+" : ""}{t("common.currencySymbol")}{stats.totalProfit.toFixed(2)} ({stats.totalProfitPercent >= 0 ? "+" : ""}{stats.totalProfitPercent.toFixed(2)}%)
          </span>
        </div>
        <div className="stat-item">
